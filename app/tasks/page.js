...
const Tasks = () => {
    ...
    const deleteAll = async () => {
        ...
        let x = await fetch('/tasks/api/d-all', {
            method: 'POST',
            headers : {
                'Content-Type' : 'application/json'
            },
    ...
    const basestyle = 'text-xs text-cyan-500 text-bold'

    return (
        <>
            <Navigation />
            <div className='md:min-h-[82vh] min-h-[79.5vh] flex flex-col items-center gap-16 bg-slate-900 p-3'>
                <h1 className='text-center bg-gradient-radial w-full p-5 border-b-2 border-red-600 from-red-600 via-red-900 to-red-300 inline-block font-ubuntu text-transparent bg-clip-text md:text-5xl text-3xl font-bold'>TaskPro&trade;</h1>
                <form ref={ref} action="" onSubmit={handleSubmit(onSubmit)} className='flex gap-3 items-center justify-center w-full'>
                    ...
                    <input disabled={isSubmitting} type="submit" value={isSubmitting ? "Adding.." : "Add"} className='disabled:opacity-20 md:text-base text-sm md:py-[0.65rem] py-[0.40rem] px-6 hover:text-cyan-300 transition-all duration-500 hover:border-cyan-300 border-2 border-slate-700 bg-slate-600/40 text-white cursor-pointer rounded-xl' />
                </form>
                ...
                <div className="tasks w-full">
                    ...
                    {tasks.length > 0 && tasks.map((item) =>  (
                        <div key={item.task_id} className="mx-auto task my-2 p-4 flex justify-between items-center md:w-1/2 w-full border-2 border-cyan-500 bg-white bg-opacity-5">
                            <span className={item.name.length < 25 ? basestyle + ' md:text-base' : basestyle + ' lg:text-base'}>{item.name}</span>
                ...
                <button disabled={tasks.length <= 0} onClick={deleteAll} className='disabled:opacity-20 p-4 bg-red-600 text-white font-extrabold rounded-lg hover:bg-red-900 hover:text-slate-300 transition-all duration-500'>Delete All</button>
            </div>
        </>
    );
};
